...
class ant_colony:
    class ant(Thread):
        def __init__(self, init_location, end_location, possible_locations, pheromone_map, distance_callback, alpha, beta, first_pass=False):
            """
        ...
        def _end_traverse(self):
            self.route.append(self.end_location)
            self._update_distance_traveled(self.location, self.end_location)
    ...
    def __init__(self, nodes, distance_callback, start=None, end=None, ant_count=50, alpha=.5, beta=1.2, pheromone_evaporation_coefficient=.40, pheromone_constant=1000.0, iterations=80):
        """
        ...
        if end is None:
            ...
            # if we didn't find a key in the nodes passed in, then raise
            if self.end is None:
                raise KeyError("Key: " + str(end) + " not found in the nodes dict passed.")
        ...
        self.ants = self._init_ants(self.start, self.end)
    ...
    def _init_ants(self, start, end):
        ...
        if self.first_pass:
            return [self.ant(start, end, self.nodes.keys(), self.pheromone_map, self._get_distance,
                             self.alpha, self.beta, first_pass=True) for x in range(self.ant_count)]
        # else, just reset them to use on another pass
        for ant in self.ants:
            ant.__init__(start, end, self.nodes.keys(), self.pheromone_map, self._get_distance, self.alpha, self.beta)
    ...
    def mainloop(self):
        ...
        for _ in range(self.iterations):
            ...
            self._init_ants(self.start, self.end)
